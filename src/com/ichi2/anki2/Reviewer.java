...
import com.ichi2.anki2.receiver.SdCardReceiver;
...
public class Reviewer extends AnkiActivity {
    ...
    private static final int MENU_RECORD = 7;
    private static final int MENU_RECORD_REPLAY = 8;
    ...
    private static final int GESTURE_RECORD = 18;
    private static final int GESTURE_RECORD_PLAY = 19;
    ...
    protected void onDestroy() {
        ...
        if (mPrefRecord) {
            Recorder.releaseTempFile();
            Recorder.setRecorderResetListener(null);
            Recorder.setRecordingFinishedListener(null);
        }
...
//    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if(mPrefRecord) {
            ...
            final MenuItem replayItem = UIUtils.addMenuItem(menu, Menu.NONE, MENU_RECORD_REPLAY, Menu.NONE, R.string.record_play, 
    ...
    private void updateCard(String content) {
        ...
        if (mCurrentSimpleInterface) {
            ...
            // If recording is enabled, the answer is being shown, a recording has been 
            // made and auto replaying is enabled
            try {
                if (mPrefRecord && sDisplayAnswer && Recorder.recorded() &&
                        mSched.getCol().getDecks().confForDid(mCurrentCard.getDid()).getBoolean("autoRecordReplay")) {
                    // TODO play before or after other media
                    Sound.addRecording(Recorder.getPath(), false, qa);
                }
            } catch (JSONException e) {
                throw new RuntimeException(e);
            }
    ...
    private void playRecording() {
        if (Recorder.recorded()) {
            Sound.stopSounds();
            Sound.playSound(Recorder.getPath(), null);
        }
    }
